...
class CmtpTimePickerView @JvmOverloads constructor(
    ...
    private fun setupRecyclersData() {
        ...
        cmtpRecyclerMinutes.apply {
            val minutes = CmtpTimeData.MINUTES.filter { it.rem(minuteStep) == 0 || it == 0 }
            recyclerMinutesAdapter.setItems(minutes.map { String.format("%02d", it) })
            val minuteIndex = minutes.indexOf(time.minute)
            recyclerMinutesLayoutManager.scrollToPosition(if (minuteIndex > 0) minuteIndex else 0)
            smoothScrollBy(0, 1)
        }
        cmtpRecyclerPmAm.apply {
